        <div class="keys">
            <div class="keysHeader headersBgCol">
                <div style="position:absolute; right: 4%; top:1.5%;">
                    <span class="close" style="font-size:22px;">X</span>
                </div>
                <div class="row">
                    <div class="col s6 m6 l6">
                        <div>
                            <img class="KHIMG" src="img/keys/wildliferehab-aqua.png">
                        </div>
                        <div>
                            <span style="font-size:14px; font-weight:bold;">
                                Wildlife Rehabilitation Center
                            </span>
                        </div>
                    </div>
                    <div class="col s6 m6 l6">
                        <!--div>
                            <img class="KHIMG" src="img/keys/vet-icon-aqua.png">
                        </div>
                        <div>
                             <span style="font-size:16px; font-weight:bold;">
                                Veterinarian
                            </span>                       
                        </div-->
                    </div>                    
                </div>
                     
            </div>
            <div class="KeyRows" id="row1">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-aqua.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">
                        <!--div>
                            <img class="KCIMG" src="img/keys/vet-icon-aqua.png">
                        </div-->
                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:#00b7bd;">
                                Currently Open
                            </span>                       
                        </div>
                    </div>
                </div>
            </div>
            <hr>
            <div class="KeyRows" id="row2">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-grey.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">
                        <!--div>
                            <img class="KCIMG" src="img/keys/vet-icon-grey.png">
                        </div-->
                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:gray;">
                                Closed
                            </span>                       
                        </div>
                    </div>
                </div>
            </div>  
            <hr>
            
            
            <!--div class="KeyRows" id="row3">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-orange.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">

                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:orange;">
                                Closing Soon
                            </span>                       
                        </div>
                    </div>
                </div>
            </div> 
            <hr>
            <div class="KeyRows" id="row4">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-red.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">

                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:red;">
                                Closed Recently
                            </span>                       
                        </div>
                    </div>
                </div>
            </div> 
            <hr>
            <div class="KeyRows" id="row5">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-pink.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">

                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:pink;">
                                Animal Specialist
                            </span>                       
                        </div>
                    </div>
                </div>
            </div>  
            <hr>
            <div class="KeyRows" id="row6">
                <div class="row">
                    <div class="col s3 m3 l3">
                        <div>
                            <img class="KCIMG" src="img/keys/wildliferehab-blue.png">
                        </div>
                    </div>
                    <div class="col s3 m3 l3">
                    </div>
                    <div class="col s6 m6 l6">
                        <div style="padding-top:9%;">
                             <span style="font-size:16px; font-weight:bold; color:blue;">
                                Opening Soon
                            </span>                       
                        </div>
                    </div>
                </div>
                
              
            </div-->
            
            <div class="keysFooter">
                <hr>
                <div style="float:right;font-size: 20px; margin:3% 9% 5% 0%;">
                    <span class='fontCol' id="Kdone"><b>DONE&#10095;</b></span>
                </div>
            </div>
        </div>